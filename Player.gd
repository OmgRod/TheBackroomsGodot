...
# (Dictionary[String] -> Array[AudioStream])
var surface_sounds := {
	"carpet": [
...
func _unhandled_input(event: InputEvent) -> void:
	...
	# --- Touch look (RIGHT half of screen only) ---
	# This guarantees multi-touch: left half can be your joystick; right half is for camera.
	var vp_size: Vector2 = get_viewport().get_visible_rect().size
	var right_half_start_x: float = vp_size.x * 0.5

	if event is InputEventScreenTouch:
		if event.pressed:
			# Assign a camera finger only if it begins on the right half
			if event.position.x >= right_half_start_x and camera_touch_index == -1:
...
func _physics_process(delta: float) -> void:
	...
	# Avoid NaN from any external source
	if is_nan(input_dir.x) or is_nan(input_dir.y):
		input_dir = Vector2.ZERO

	# Convert 2D to 3D relative to the neck yaw
...
func _update_crouch_state(delta: float) -> void:
	...
	# Smoothly scale capsule and neck position
...
# --- CAMERA BOBBING ---
func _apply_camera_bobbing(delta: float, speed_len: float, sprinting: bool, crouching: bool) -> void:
	...
	if result and result.collider:
		...
		if node and node.has_meta("MaterialType"):
			...
			if material in surface_sounds and direction.length() > 0.1 and is_on_floor():
				if footstep_cooldown <= 0.0:
					# Safe fetch and guard against empty
					var sounds: Array = surface_sounds.get(material, [])
					if sounds.size() > 0:
						var idx := _rng.randi_range(0, sounds.size() - 1)
